...
def main():
    ...
    for item in root.findall('.//cwe:View', NS):
        status = item.attrib.get('Status', '')
        if 'Deprecated' in status:
            continue

        view_id = f"View-{item.attrib.get('ID', '')}"
        name = item.attrib.get('Name', '')
        view_type = item.attrib.get('Type', '')

        has_member = []
        for members in item.findall('.//cwe:Members', NS):
            for member in members.findall('.//cwe:Has_Member', NS):
