...
output_file = output_dir / 'cwe_view.csv'
...
def extract_has_member(view):
    ...
    for has_member in members.findall('cwe:Has_Member', ns):
...
def main():
    ...
    for view in root.findall('.//cwe:View', ns):
        status = view.attrib.get('Status', '').strip()

        if status == 'Obsolete':
            continue

        view_id = view.attrib.get('ID', '').strip()
        name = view.attrib.get('Name', '').strip()
        has_member = extract_has_member(view)
        ...
        rows.append({
            'view_id': f'View-{view_id}',
            'name': name if name else '*',
            'status': status if status else '*',
            'has_member': has_member,
        })

    df = pd.DataFrame(rows, columns=['view_id', 'name', 'status', 'has_member'])

    df = df.sort_values(
        by='view_id',
        key=lambda s: s.str.extract(r'View-(\d+)')[0].astype(int)
